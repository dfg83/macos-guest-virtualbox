...
vmname="high_sierra"             # name of the VirtualBox virtual machine
...
printf '
  One-key semi-automatic installation of macOS On VirtualBox -  ‎High Sierra 10.13.6
-------------------------------------------------------------------------------
...
# Initialize the VirtualBox macOS ‎High Sierra 10.13.6 virtual machine config:
...
if [ -r "BaseSystem.vdi" ]; then
    ...
    wget -c 'http://swcdn.apple.com/content/downloads/29/03/091-94326/45lbgwa82gbgt7zbgeqlaurw2t9zxl8ku7/BaseSystem.dmg' -O "BaseSystem.dmg" 2>/dev/tty
...
if [ -r "${vmname}.vdi" ]; then
    ...
    echo "Attempting to install macOS High Sierra on a disk smaller than 22000MB will fail."
...
promptterminalready
echo ""
echo "Downloading macOS ‎High Sierra 10.13.6 installer."

# downloading macOS
kbstring='urlpath="http://swcdn.apple.com/content/downloads/29/03/091-94326/45lbgwa82gbgt7zbgeqlaurw2t9zxl8ku7/"; for filename in BaseSystem.chunklist InstallInfo.plist AppleDiagnostics.dmg AppleDiagnostics.chunklist BaseSystem.dmg InstallESDDmg.pkg; do curl "${urlpath}${filename}" -o "/Volumes/'"${vmname}"'/${filename}"; done'
sendkeys
promptterminalready
echo ""
...
kbstring='mount -rw / && installpath="/Install macOS High Sierra.app/Contents/SharedSupport/" && mkdir -p "${installpath}" && cd "/Volumes/'"${vmname}/"'" && mv *.chunklist *.plist *.dmg *.pkg "${installpath}"'
sendkeys
...
# Start the installer.
kbstring='cd "/Install macOS High Sierra.app/Contents/Resources/"; ./startosinstall --volume "/Volumes/'"${vmname}"'"'
sendkeys
printf ${whiteonblack}'
...
printf '
macOS High Sierra 10.13.6 will now install and start up.
...
printf 'macOS ‎High Sierra 10.13.6 installation should complete in a few minutes.
